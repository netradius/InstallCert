...
/**
 * All code in this class was originally based on a post at http://www.dzone.com/snippets/ssl-download-certificate-chain.
 *
...
public class InstallCert {
	...
	private static void addCertificateChain(KeyStore ks, X509Certificate[] chain, String host, boolean noprompt) {
		try {
			...
			System.err.println("an error occurred adding certiticate to the keystore: " + x.getMessage());
			x.printStackTrace();
			System.exit(1);
		}
	}
	...
	private static void backup(File keyStoreFile) {
		...
		try {
			System.out.println("backing up " + keyStoreFile.getAbsolutePath() + " to " + bak.getAbsolutePath());
